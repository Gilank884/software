        ...
        {PRODUCTS.map((product) => (
          <button
            ...
            className={`bg-white/90 backdrop-blur-3xl p-8 rounded-[50px] border-4 transition-all group relative overflow-hidden text-left flex items-center justify-between gap-6 ${
              product.popular ? 'border-blue-100 shadow-xl scale-105 z-10' : 'border-white shadow-lg hover:border-blue-50'
            }`}
          >
            {product.popular && (
              <div className="absolute top-4 right-4 bg-blue-600 text-white px-3 py-1 rounded-full flex items-center gap-1 text-[8px] font-black tracking-widest font-sans z-10">
            ...
            <div className="flex-1">
              ...
              <div className="text-4xl font-black text-gradient-blue leading-none">
                Rp {product.price.toLocaleString('id-ID')}
              </div>
            </div>

            <div className={`w-16 h-16 bg-blue-50 rounded-[20px] flex items-center justify-center transition-all duration-500 group-hover:scale-110 shadow-inner group-hover:bg-blue-600 group-hover:text-white`}>
