...
const CanvasManager = ({ user }) => {
  ...
  const handleUpdateEraserGuide = (rect) => {
    setEraserGuide(rect)
    // Automatically analyze colors when moved/resized
  ...
  const analyzeAreaPalette = async (rect, shouldClose = true) => {
    ...
    if (thumbnailCanvasRef.current) {
      ...
      extractPaletteFromData(data, shouldClose)
      return
    }

    // Fallback to full image if thumbnail isn't ready
    try {
      const img = new Image()
      img.crossOrigin = "anonymous"
      img.src = activeFrame.url
  ...
  const extractPaletteFromData = (data, shouldClose) => {
    const colorCounts = {}
    // Sample more densely since it's a small thumbnail
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-300">
      ...
      <p className="font-black uppercase tracking-[0.3em] text-[10px] text-slate-400">Loading Canvas...</p>
    </div>
  )
            ...
            <button
                onClick={() => {
                  if (eraserGuide) {
                    ...
                    setEraserGuide({ x: 150, y: 300, width: 300, height: 300 })
                  }
                }}
                className={`group relative flex items-center justify-center w-12 h-12 rounded-2xl font-black text-[9px] uppercase tracking-widest transition-all ${eraserGuide ? 'bg-amber-600 text-white shadow-lg shadow-amber-300' : 'hover:bg-white text-slate-600 hover:shadow-sm'}`}
                title="Gunting Masal"
              >
                <Maximize2 size={18} className={eraserGuide ? 'animate-pulse' : ''} />
                <span className="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-slate-900/90 backdrop-blur-md text-white text-[8px] font-black uppercase tracking-[0.2em] rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-[100] shadow-2xl border border-white/10 translate-y-2 group-hover:translate-y-0">
                  Gunting Masal
                </span>
            </button>
        ...
        <div className="flex-1 flex flex-col p-6 overflow-y-auto custom-scrollbar gap-8">
          ...
          <div className="space-y-3">
             <label className="group flex items-center gap-3 w-full px-4 py-3 bg-white/40 border border-slate-200 hover:border-blue-500 rounded-2xl cursor-pointer transition-all">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-blue-600">Import Frame</span>
                <input type="file" className="hidden" onChange={handleFileUpload} accept="image/*" />
              </label>

              <button
                onClick={addSlot}
                className="w-full flex items-center gap-3 px-4 py-3 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 disabled:opacity-30 transition-all shadow-lg"
              >
